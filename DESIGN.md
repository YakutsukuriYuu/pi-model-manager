...
接入条目省略 `api` 时，协议从 Pi 的内置接入继承。早期实现直接在列里打印「继承内置」—— 等于把一个用户需要知道的值藏起来。现在显示实际生效的协议并标 `*`，并在表头下一行说明 `*` 的含义。
...
## 覆盖 models.json 的全部字段

「超集」不能只覆盖常用字段。models.json 里模型级别还有 `cost`、`thinkingLevelMap`、`samplingParams`、`promptCache`、`headers`、`compat`，接入级别还有 `compat`、`modelOverrides`、`oauth`—— 为每个都做一行表单既冗长又把 Pi 的 schema 硬编码进 UI。
...
三条规则让它不会变成“两个地方改同一个值”：

1. 写在 JSON 行的、已由表单拥有的键会被**拒绝**并给出提示，而不是静默忽略
2. JSON 行**替换**而非合并：不在其中的键会被删掉，所以删一个键就是真的删
3. `extras` 在草稿类型上是**必填**的。如果做成可选，任何一个忘了传的构造点都会在保存时把用户的 `cost` 抹掉 —— 这种错必须由类型系统挡住，而不是靠记得

由于长度可能超过一行，内联编辑器改为显示**尾部**（`tailToWidth`）：普通截断删掉的恰好是用户正在输入的字符。

## 归属模型：三种存储，一条归属线

用户要的是「Pi 模型配置的超集」，所以得想清楚一个模型到底存在哪里：

| 归属 | 存储 | 谁拥有 |
| --- | --- | --- |
| 配置 | `providers[x].models[]` | 用户 |
| 覆盖 | `providers[x].modelOverrides[y]` | 用户（字段级，其余仍继承） |
| 继承 | Pi 生成的目录 | Pi |

「继承」不是一个可以长期存在的状态，而是**尚未接管**。按下 `e` 编辑继承模型时，插件直接走接管（写一条完整配置），而不是给用户塞一个 `modelOverrides` 概念 —— 后者虽然能保持同步，但用户明确表达了他要的是「完全拥有」。

`modelOverrides` 仍然被**读取**（手工写的配置、旧数据不会丢，也能删），但新建时不再产生它。

### 接管为什么可逆

删掉接管后的条目，Pi 的目录模型会重新出现 —— 也就是说接管只是取了一份拷贝，不是破坏性操作。这是敢把它做成 `e` 默认行为的前提。

代价是接管后数值会冻结（Pi 以后更新目录也影响不到），且清空字段会回到 Pi 默认值而不是目录值。表单把这句话写在用户眼前，而不是靠文档。
